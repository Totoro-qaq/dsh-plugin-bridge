...
Rules: drop details tied to the old preset's tools; keep decision rationale; every path must come from the material, never invent one; copy exact numbers (ports, versions, limits) verbatim on their own line so they cannot be rounded to common values; no pleasantries — output the summary only.
...
要求：删去与原模式工具细节相关的内容；保留决策理由；路径必须来自取材，不得编造；端口、版本号、数量上限这类精确数字必须原样单独成行抄写，不得改写成常见值；不要寒暄，直接输出摘要。
