 ...
 */
const SECTION_SHARE = { compaction: 0.25, users: 0.45, recent: 0.3 } as const;
...
/** 一个可按预算裁剪的取材分区。 */
interface Section {
  name: keyof typeof SECTION_SHARE;
  header: string;
  items: string[];
  /** newest：从最新的一条开始保留；head：单块内容从头部保留。 */
  keep: 'newest' | 'head';
  /** 被丢弃条目的提示语（keep === 'newest' 时使用）。 */
...
function renderSection(section: Section, budget: number): RenderedSection | null {
  const header = section.header;
  if (budget <= header.length + 1) return null;
  const room = budget - header.length - 1; // -1: 表头与正文之间的换行

  if (section.keep === 'head') {
    const body = section.items.join('\n');
    if (body.length <= room) return { text: `${header}\n${body}`, clipped: false };
    const marker = '\n…（底稿截断）';
    // 只放得下截断标记本身就没有信息量了，整段让位给优先级更高的分区。
    if (room <= marker.length) return null;
  ...
  for (; index >= 0; index -= 1) {
    const item = section.items[index] as string;
  ...
  if (droppedCount > 0) {
    ...
    while (picked.length > 1 && used + note.length + 1 > room) {
      const shed = picked.shift() as string;
      used -= shed.length + 1;
    }
    return { text: `${header}\n${note}\n${picked.join('\n')}`, clipped: true };
  }
...
export function buildBridgeSource(messages: ChatMessage[], options: BridgeSourceOptions = {}): BridgeSource {
  ...
  const assistants = messages.filter(
    (m) => m.role === 'assistant' && (m.content.trim() || (m.toolNodes?.length ?? 0) > 0),
  );
  ...
  for (const name of ['users', 'recent', 'compaction'] as const) {
    if (pool <= 0) break;
  ...
  for (const section of sections) {
    ...
    if (rendered.clipped) dropped.push(section.name);
...
type UserConstraintKind = 'tools' | 'files';
 ...
 */
export function preserveActiveUserConstraints(
  summary: string,
  messages: ChatMessage[],
  lang: 'zh' | 'en',
): string {
  const patterns: Record<UserConstraintKind, { ban: RegExp[]; allow: RegExp[]; alreadyPresent: RegExp }> = {
    tools: {
      ban: [
        /(?:^|[，,。！？;；\n])\s*(?:但|不过|仍|还是)?\s*((?:不要|禁止|不得|不能|不准|别)(?:调用|使用)任何工具)/gu,
        /(?:^|[，,。！？;；\n])\s*(?:但|不过|仍|还是)?\s*((?:不要|禁止|不得|不能|不准|别)调用工具)/gu,
    ...
    files: {
      ban: [
        /(?:^|[，,。！？;；\n])\s*(?:但|不过|仍|还是)?\s*((?:不要|禁止|不得|不能|不准|别)(?:读写|读取|写入|修改)文件)/gu,
  ...
  for (const message of messages) {
    ...
    for (const kind of ['tools', 'files'] as const) {
      const events: { at: number; value?: string }[] = [];
      for (const pattern of patterns[kind].ban) {
        for (const match of message.content.matchAll(pattern)) {
          events.push({ at: match.index, value: match[1] });
        }
      }
      for (const pattern of patterns[kind].allow) {
        for (const match of message.content.matchAll(pattern)) events.push({ at: match.index });
      }
      events.sort((a, b) => a.at - b.at);
      for (const event of events) {
        if (event.value) active.set(kind, event.value);
        else active.delete(kind);
      }
    }
  }
  const missing = (['tools', 'files'] as const)
    .filter((kind) => active.has(kind) && !patterns[kind].alreadyPresent.test(summary))
    .map((kind) => `- ${active.get(kind)}`);
  if (!missing.length) return summary;
  const heading = lang === 'zh' ? '## 关键决策与约定' : '## Key decisions & conventions';
  const position = summary.indexOf(heading);
  if (position < 0) return `${summary.trimEnd()}\n\n${heading}\n${missing.join('\n')}`;
  const insertAt = position + heading.length;
...
export function summaryTokenBudget(summaryCharBudget: number = SUMMARY_CHAR_BUDGET): number {
  return Math.max(100, Math.round(summaryCharBudget / SUMMARY_CHARS_PER_TOKEN));
}

/** 压缩指令：让工人模型输出固定 schema 的交接摘要。 */
export function buildBridgeInstruction(lang: 'zh' | 'en', options: BridgeInstructionOptions = {}): string {
...
export function estimateSummaryTokens(
  sourceChars: number,
  options: BridgeInstructionOptions = {},
