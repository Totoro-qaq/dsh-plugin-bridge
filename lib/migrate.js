 ...
 * 「被交付的」是同一条代码路径。
 */
import { buildBridgeInstruction, buildBridgeKickoff, buildBridgeSource, preserveActiveUserConstraints, appendVisualEvidence, collectVisualEvidence, detectLang, } from './compression.js';
...
function workerFailure(settled, summary, bounds) {
    const end = settled.end;
    const fail = (code, message, details) => new RpcError('bridge.preview', code, message, details);
...
export async function previewMigration(input, options) {
    ...
    try {
        ...
        if (failure)
            throw failure;
        const summary = appendVisualEvidence(preserveActiveUserConstraints(workerSummary, messages, lang), source.visualEvidence, lang);
        return {
            summary,
            source,
            lang,
