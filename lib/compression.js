 ...
 */
export function preserveActiveUserConstraints(summary, messages, lang) {
    const patterns = {
        tools: {
            ban: [
            ...
            alreadyPresent: /(?:不要|禁止|不得|不能|不准|别)(?:调用|使用)(?:任何)?工具|do not (?:use|call) any tools/iu,
        },
        files: {
            ban: [
            ...
            alreadyPresent: /(?:不要|禁止|不得|不能|不准|别)(?:读写|读取|写入|修改)文件|do not (?:read or write|read|write|modify) (?:any )?files/iu,
        },
    };
    ...
    for (const message of messages) {
        ...
        for (const kind of ['tools', 'files']) {
            ...
            for (const pattern of patterns[kind].allow) {
                for (const match of message.content.matchAll(pattern))
                    events.push({ at: match.index });
            }
            events.sort((a, b) => a.at - b.at);
            for (const event of events) {
                if (event.value)
                    active.set(kind, event.value);
                else
                    active.delete(kind);
            }
        }
    }
    const missing = ['tools', 'files']
