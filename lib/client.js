window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		function parseBridgeTargets(text) {
			const lines = text.split("\n");
			for (const lang of ["zh", "en"]) {
				...
				if (available.length !== 1 || currentLines.length > 1) return void 0;
				const targets = (available[0] ?? "").slice(copy.available.length).split(" · ");
				if (!targets.every((id) => SAFE_PRESET_ID.test(id)) || new Set(targets).size !== targets.length) return void 0;
				const current = currentLines[0]?.slice(copy.current.length);
				if (current !== void 0 && (!SAFE_PRESET_ID.test(current) || targets.includes(current))) return void 0;
				return {
					lang,
					targets,
		...
		const NAVIGATION_COPY = {
			...
			en: {
				...
				failed: (sessionId, reason) => `Could not open target session ${sessionId}: ${reason}`,
				notVisible: (sessionId) => `Target session ${sessionId} has not reached this browser yet.`
			}
		};
		function isSessionNavigator(value) {
			return (typeof value === "object" || typeof value === "function") && value !== null && "openSession" in value && typeof value.openSession === "function";
		}
		/** Resolve the host navigation service when it is active; never throws for an absent service. */
		function bridgeSessionNavigator(ctx) {
			if (typeof ctx.get !== "function") return void 0;
			try {
				const service = ctx.get(BRIDGE_NAVIGATION_SERVICE);
				return isSessionNavigator(service) ? service : void 0;
			} catch {
				return;
			}
		}
		/** The routes this host offers, in the order Bridge tries them. */
		function bridgeSessionRoutes(ctx) {
			const routes = [];
			if (bridgeSessionNavigator(ctx) !== void 0) routes.push("uiWorkspace.openSession");
		...
		*/
		function openBridgeSession(ctx, sessionId, lang) {
			...
			if (typeof sessions.open === "function") try {
				sessions.open(sessionId);
				return "sessions.open";
			} catch (cause) {
				failure ??= cause;
			}
			if (failure === void 0) throw new Error(copy.unavailable);
			throw new Error(copy.failed(sessionId, errorMessage(failure)), { cause: failure });
		}
		function abortReason(signal) {
			return signal.reason ?? /* @__PURE__ */ new Error("Bridge session navigation was cancelled.");
		}
		/** Resolve once `sessionId` is listed; reject on timeout or abort, always releasing the subscription and timer. */
		function waitForBridgeSession(list, sessionId, { lang, signal, timeoutMs = BRIDGE_SESSION_VISIBLE_TIMEOUT_MS }) {
			const listed = () => Reflect.get(list.getSnapshot().byId, sessionId) !== void 0;
			...
			return new Promise((resolve, reject) => {
				...
				const onAbort = () => {
					if (signal !== void 0) finish(abortReason(signal));
				};
				const timer = setTimeout(() => {
					finish(new Error(NAVIGATION_COPY[lang].notVisible(sessionId)));
				}, timeoutMs);
		...
		*/
		async function openBridgeSessionWhenVisible(ctx, sessionId, options) {
		...
		*/
		function buildBridgePreviewCommand(targetPreset, lang) {
		...
		function buildBridgeMigrationCommand(targetPreset, summary, lang, previewId, options = {}) {
			if (!SAFE_PRESET_ID.test(targetPreset)) throw new Error("Unsupported target preset id");
			...
			return `/bridge ${targetPreset} --go --lang ${lang} --preview-id ${previewId}${options.autoContinue === true ? " --continue" : ""} --summary64 ${encodeUtf8Base64Url(summary)}`;
		}
		//#endregion
...
@media(max-width:900px){.dsh-bridge-preview-card{margin-bottom:clamp(96px,18vh,150px)}}
...
`;
		const COPY = {
			zh: {
				...
				waitMode: "等待我确认",
				continueMode: "直接继续",
				waitHelp: "新会话复述交接后停下，等你确认。",
				continueHelp: "新会话复述交接后，在同一轮接着做下一步。",
				pickTarget: "选一个模式生成交接预览：",
				pickRunning: "正在生成预览，结果会出现在下方的新卡片里。",
				pickDone: "预览已生成，见下方的新卡片。"
			},
			en: {
				...
				waitMode: "Wait for my confirmation",
				continueMode: "Continue directly",
				...
				pickTarget: "Pick a preset to preview a handoff:",
				pickRunning: "Generating the preview; it appears in a new card below.",
				pickDone: "The preview is ready in the new card below."
			}
		};
		...
		function PreviewCard({ card, execute, openSession, sessionId }) {
			...
			const [autoContinue, setAutoContinue] = (0, react.useState)(false);
			...
			const confirm = async () => {
				...
				try {
					const result = parseBridgeCard(await execute(sessionId, buildBridgeMigrationCommand(card.targetPreset, summary, card.lang, card.previewId ?? "", { autoContinue })));
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: "dsh-bridge-card dsh-bridge-preview-card",
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								...
								children: [
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										className: "dsh-bridge-after",
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
											className: "dsh-bridge-field-help",
											children: copy.afterMigration
										}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
											className: "dsh-bridge-tabs",
											role: "radiogroup",
											"aria-label": copy.afterMigration,
											children: [[
												false,
												copy.waitMode,
												copy.waitHelp
												...
												copy.continueMode,
												copy.continueHelp
											]].map(([value, label, help]) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
												className: "dsh-bridge-choice",
												title: help,
												children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
													type: "radio",
													name: `${panelId}-after`,
													checked: autoContinue === value,
													disabled: busy,
													onChange: () => {
		...
		function MigratedCard({ card, error = "", openSession, status = "" }) {
			...
			const open = async () => {
				...
				setLocalError("");
				try {
					await openSession(card.sessionId, card.lang);
				} catch (cause) {
		...
		function TargetPicker({ execute, picker, sessionId }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: "dsh-bridge-picker",
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: "dsh-bridge-field-label",
						children: copy.pickTarget
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: "dsh-bridge-choices",
						children: picker.targets.map((target) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							className: "dsh-bridge-button",
							disabled: running,
							type: "button",
							onClick: () => {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Header, { lang }), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						...
						children: card.phase === "error" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							...
							className: "dsh-bridge-message",
							children: card.text
						})
					}), card.picker ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(TargetPicker, {
						execute,
						picker: card.picker,
						sessionId
					}) : null]
		...
		function BridgeCommandCardContent({ node, execute, openSession, sessionId }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(MessageCard, {
				card,
				execute,
				sessionId
			});
		...
		function apply(ctx) {
			...
			ctx.effect(() => () => {
				navigation.abort();
			}, "bridge: target session navigation");
			ctx.effect(() => {
			...
			ctx.slots.inject("conversation.chat.commandview", () => ctx.slots.register({
				...
				inject: () => ({
					...
					openSession: async (sessionId, lang) => {
						await openBridgeSessionWhenVisible(ctx, sessionId, {
							lang,
