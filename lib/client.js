window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function hasUnsafeBlock(line) {
			return MARKDOWN_BLOCK_START.test(line) || THEMATIC_OR_SETEXT.test(line) || TABLE_DELIMITER.test(line) || LINK_DEFINITION.test(line) || /^(?: {4}|\t)/u.test(line) || /<!--|-->/u.test(line) || HTML_BLOCK_START.test(line);
		}
		...
		*/
		function parseBridgeTextProjection(markdown) {
			...
			if (/<!--|-->/u.test(editableMarkdown)) return void 0;
			const sections = [];
