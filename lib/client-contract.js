 ...
 */
export function parseBridgeTargets(text) {
    const lines = text.split('\n');
    for (const lang of ['zh', 'en']) {
        ...
        if (available.length !== 1 || currentLines.length > 1)
            return undefined;
        const targets = (available[0] ?? '').slice(copy.available.length).split(' · ');
        if (!targets.every((id) => SAFE_PRESET_ID.test(id)) || new Set(targets).size !== targets.length)
            return undefined;
        const current = currentLines[0]?.slice(copy.current.length);
        if (current !== undefined && (!SAFE_PRESET_ID.test(current) || targets.includes(current)))
            return undefined;
...
export function parseBridgeCard(outcome) {
    ...
    if (outcome.kind === 'error')
        return { phase: 'error', text, ...(picker ? { picker } : {}) };
...
export function bridgeSessionNavigator(ctx) {
...
export function bridgeSessionRoutes(ctx) {
    const routes = [];
    if (bridgeSessionNavigator(ctx) !== undefined)
        routes.push('uiWorkspace.openSession');
    if (typeof ctx.sessions.open === 'function')
        routes.push('sessions.open');
    return routes;
}
 ...
 */
export function openBridgeSession(ctx, sessionId, lang) {
...
export function waitForBridgeSession(list, sessionId, { lang, signal, timeoutMs = BRIDGE_SESSION_VISIBLE_TIMEOUT_MS }) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const onAbort = () => { if (signal !== undefined)
        ...
        const stop = list.subscribe(() => { if (listed())
            finish(); });
        if (settled)
            stop();
        else
 ...
 */
export async function openBridgeSessionWhenVisible(ctx, sessionId, options) {
    if (bridgeSessionRoutes(ctx).length === 0)
        throw new Error(NAVIGATION_COPY[options.lang].unavailable);
 ...
 */
export function buildBridgePreviewCommand(targetPreset, lang) {
    if (!SAFE_PRESET_ID.test(targetPreset))
...
export function buildBridgeMigrationCommand(targetPreset, summary, lang, previewId, options = {}) {
    if (!SAFE_PRESET_ID.test(targetPreset))
        throw new Error('Unsupported target preset id');
    if (lang !== 'zh' && lang !== 'en')
